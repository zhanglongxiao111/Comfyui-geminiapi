...
class NanoBananaProVertexNode:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "project_id_input": ("STRING", {"multiline": False, "default": ""}),
                "location_input": ("STRING", {"multiline": False, "default": ""}),
                "service_account_input": ("STRING", {"multiline": False, "default": ""}),
                "image_count_input": (
                    "INT",
    ...
    def _create_client(
        ...
        if not project_id:
            raise ValueError("Project ID is required for Vertex.")
        if not location:
            raise ValueError("Location is required for Vertex.")
    ...
    def _generate_once(
        ...
        client = self._create_client(service_account, project_id, location)
        ...
        if not images:
            print("[Nano Banana Pro Vertex] API returned no images; using fallback frame.")
        return images
    ...
    def _accumulate_results(
        ...
        for item in generator:
            try:
                ...
                print(f"[Nano Banana Pro Vertex] Request failed: {self._describe_exception(exc)}")
                errors.append(exc)
                continue
        ...
        if len(results) < desired:
            ...
            if errors:
                print(
                    f"[Nano Banana Pro Vertex] Encountered {len(errors)} errors; "
    ...
    def generate(
        ...
        resolved_project = self._resolve_string(project_id, project_id_input)
        resolved_location = self._resolve_string(location, location_input)
        resolved_service_account = self._resolve_string(service_account, service_account_input)
    ...
    def IS_CHANGED(cls, **kwargs):
        return (
            f"{kwargs.get('project_id', '')}-"
            f"{kwargs.get('location', '')}-"
            f"{kwargs.get('service_account', '')}-"
            f"{kwargs.get('prompt', '')}-"
...
NODE_CLASS_MAPPINGS = {"NanoBananaProVertexNode": NanoBananaProVertexNode}
NODE_DISPLAY_NAME_MAPPINGS = {"NanoBananaProVertexNode": "Nano Banana Pro (Vertex)"}
